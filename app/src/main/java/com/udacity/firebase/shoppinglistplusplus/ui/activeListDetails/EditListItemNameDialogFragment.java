...
public class EditListItemNameDialogFragment extends EditListDialogFragment {
    ...
    protected void doListEdit() {
        ...
        //set input text to be the current list item name if it is not empty and is not hte previous name
        if(!nameInput.equals("") && !nameInput.equals(mItemName)) {
            Firebase firebaseRef = new Firebase (Constants.FIREBASE_URL);

            //make map for the item you are editing the name of
            HashMap<String, Object> updatedItemToAddMap = new HashMap<String, Object>();

            //Add the new name to the update map    FIRBASE_LOCATION_SHOPPING_LIST_ITEMS
            updatedItemToAddMap.put("/" + Constants.FIREBASE_LOCATION_SHOPPING_LIST_ITEMS + "/" + mListId + "/" +
                    mItemId + "/" + Constants.FIREBASE_PROPERTY_ITEM_NAME, nameInput);
